...
const ChainLinkV2Address = process.env.CHAINLINK_ORACLE_V2_ADDRESS
...
describe("Oracle peek()", function () {
    it("Should return valid price feed", async () => {
        ...
        chainlinkV2Contract= await ethers.getContractAt("IChainLinkPriceOracle", ChainLinkV2Address)
        ...
        expect(oracleData).to.be.equal(testOracleData)
        oracleReturnData = await chainlinkV2Contract.peek(oracleData)
