...
contract Registry is IRegistry, Shared, ReentrancyGuard {
    // Constant public
    uint256 public constant GAS_OVERHEAD_AUTO = 16000;
    uint256 public constant GAS_OVERHEAD_ETH = 6000;
    uint256 public constant BASE_BPS = 10000;
    uint256 public constant PAY_AUTO_BPS = 11000;
    uint256 public constant PAY_ETH_BPS = 13000;
    ...
    event HashedReqExecuted(uint256 indexed id, bool wasRemoved);
    event HashedReqCancelled(uint256 indexed id);
    ...
    function newReq(
        ...
        bool insertFeeAmount,
        bool isAlive
    ) external payable override targetNotThis(target) returns (uint256 id) {
        return
            _newReq(
                target,
                referer,
    ...
    function _newReq(
        ...
        bool payWithAUTO,
        bool isAlive
    ) private validEth(payWithAUTO, ethForCall) returns (uint256 id) {
        if (isAlive) {
    ...
    function getHashedReqsSlice(uint256 startIdx, uint256 endIdx)
        external
        view
        override
    ...
    function getHashedReqsLen() external view override returns (uint256) {
        return _hashedReqs.length;
    }

    function getHashedReq(uint256 id) external view override returns (bytes32) {
    ...
    function getReqBytes(Request memory r)
        public
        pure
        override
        ...
        returns (uint256 gasUsed)
    {
        ...
        gasUsed =
            startGas -
            gasleft() +
            (r.payWithAUTO == true ? GAS_OVERHEAD_AUTO : GAS_OVERHEAD_ETH);
    ...
    function _execute(Request calldata r, uint256 expectedGas) private {
        IOracle orac = _oracle;
        uint256 ethStartBal = address(this).balance;
        uint256 feeTotal;
        if (r.payWithAUTO) {
            feeTotal =
                (expectedGas *
                    orac.getGasPriceFast() *
                    orac.getAUTOPerETH() *
                    PAY_AUTO_BPS) /
                (BASE_BPS * _E_18);
        } else {
            feeTotal =
                (expectedGas * orac.getGasPriceFast() * PAY_ETH_BPS) /
                BASE_BPS;
        }
        ...
        if (r.verifyUser && !r.insertFeeAmount) {
            (success, returnData) = _userForwarder.forward{value: r.ethForCall}(
                r.target,
                r.callData
            );
            ...
            (success, returnData) = _userGasForwarder.forward{
                value: r.ethForCall
            }(r.target, insertToCallData(r.callData, feeTotal, 36));
        } else {
            (success, returnData) = r.target.call{value: r.ethForCall}(
                r.callData
            );
        }
        ...
        uint256 ethReceivedDuringRequest = address(this).balance +
            r.ethForCall -
            ethStartBal;
        if (r.payWithAUTO) {
    ...
    function cancelHashedReq(uint256 id, Request memory r)
        external
        override
        nonReentrant
        verReq(id, r)
    ...
    function getReqCountOf(address addr)
        external
        view
        override
        returns (uint256)
    ...
    function getExecCountOf(address addr)
        external
        view
        override
        returns (uint256)
    ...
    function getReferalCountOf(address addr)
        external
        view
        override
        returns (uint256)
    ...
    function _getBytes32Slice(
        ...
        uint256 sliceIdx = 0;
        for (uint256 arrIdx = startIdx; arrIdx < endIdx; arrIdx++) {
    ...
    modifier validEth(bool payWithAUTO, uint256 ethForCall) {
        if (payWithAUTO) {
    ...
    modifier validCalldata(Request calldata r) {
        if (r.verifyUser) {
            require(
                abi.decode(r.callData[4:36], (address)) == r.user,
                "Reg: calldata not verified"
            );
        }
        _;
    }
    ...
    modifier validExec() {
        require(
            _stakeMan.isUpdatedExec(msg.sender),
            "Reg: not executor or expired"
        );
        _;
    }
    ...
    modifier verReq(uint256 id, Request memory r) {
        require(
